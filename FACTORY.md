...
- **[2026-07-13]**: Fixed native GY906 (temperature) presence — surfaced by an external review ("native GY906 doesn't work"). Root cause: `BleForegroundService.kt`'s `onCharacteristicChanged` only decoded the 16-byte MAX30100 pulse packet; the 14-byte GY906 temp packet was re-broadcast to Dart but never processed natively, and every background presence consumer (sync-seconds tally, pet-care `isSynced`, `ACTION_QUERY_STATUS`, connect broadcast) read `bioProcessor.humanDetected` — always false for a temperature device. Net effect: with a GY906, cloud sync / mission tallies / pet-care all dead whenever the Flutter engine was suspended (the native service is the background source of truth), while the in-app Dart path looked fine. Added a native `TemperatureSignalProcessor` mirroring `lib/services/device/temperature_signal_processor.dart` (rawToCelsius `raw*0.02-273.15`, 29.7–41 °C forearm range, 0.5 s sustained, 15 s grace matching `BioSignalProcessor`, `rawTemp==0`→absent, writes `native_human_detected`), a sticky `activeSensorBytes` type latch (ADR 0006) reset on disconnect, a 14-byte decode branch (`getShort(12) and 0xFFFF`, matches firmware `payload[6]`), and `presenceDetected()`/`evaluateActivePresenceGrace()` facades routing every consumer to the active sensor's processor. Strict/always-on only — the lenient duty-cycle profile stays Dart-side (current firmware always-on). `cavecrew-reviewer` adversarial pass: 1 crit (stale grace window on reconnect → fixed with `tempProcessor.reset()` on disconnect) + 2 cross-thread visibility risks (fixed with `@Volatile`), all resolved. `flutter test` 84/84 green (Dart untouched); native compiled via `flutter build apk --debug`. Not yet verified on real GY906 hardware — folds into the existing Phase 6 S3 sticky-type re-test item. Deferred sticky-swap bug (`device-switch-sticky-type`) not addressed, though the new disconnect-time latch reset should help it.
