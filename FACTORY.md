...
- **[2026-07-11]**: Consolidated the stalled release pipeline — promoted a month of `unstable`-only work (native wakelock sync fix, ADRs 0007-0011, treatment/game allowlist, docs, `sync_session` removal) down `unstable → dev → main` and cut stable **v1.1.x**. Cherry-picked main's `create-pull-request` CI fix back onto `unstable` first so the promotions were conflict-free apart from `FACTORY.md`/`pubspec.yaml`. `sync_session` (the old Dart usage tracker) is now gone from `main`; native per-minute `sync_status` is the sole usage-truth — backend notified.
